...
class MixingCalculatorView : UIView, UITextFieldDelegate
{
    ...
    func buildWeightBasedStacks() {
        var numElementsPerRow = 6
        if DataManager.instance.fontSize >= 14 && DataManager.instance.fontSize < 16 {
            numElementsPerRow = 5
        } else if DataManager.instance.fontSize >= 16 {
            numElementsPerRow = 4
        }
        let numRows : Int = (numElements % numElementsPerRow == 0) ? (numElements / numElementsPerRow) : (numElements / numElementsPerRow) + 1
    ...
    func buildNonWeightBasedStacks() {
        ...
        for dose in dose_array {
            ...
            if(greyCell)
            {
                hDoseSubStack.addBackground(color: UIColor(named: "Secondary_background")!)
            } else {
                hDoseSubStack.addBackground(color: UIColor(named: "Primary_background")!)
            }
            greyCell = !greyCell
