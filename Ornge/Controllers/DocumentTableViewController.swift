...
class CellContent {
    ...
    {
        if let color = getColorFromTaggedString(string, tag: "<color=")
        {
    ...
    func isHexString(_ inputString : String) -> Bool {
        if(inputString.hasPrefix("#")) { return true }
        
        return false
    }
    
    func getColorFromTaggedString(_ inputString : String, tag : String) -> UIColor? {
        ...
        if let tagStartIndex = tempString.index(of: tag) {
            ...
            if let tagEndIndex = tempString.firstIndex(of: ">") {
                ...
                if(isHexString(String(subString))) {
                    return UIColor(hex: String(subString))
                } else {
                    return UIColor(named: String(subString))
                }
            }
        }
...
class CellContentAttributedString : CellContent {
    ...
    override init(_ string : String) {
        ...
        if let color = getColorFromTaggedString(string, tag: "<text_color=")
        {
            cellTextColor = color
        }
    }
        ...
        if let sectionRow = indexedCollectionView.cells[indexPath.section] as? NSMutableArray {
            if let cellInfo = sectionRow[indexPath.item] as? CellInfo {
                ...
                case .AttributedString:
                    ...
                    cell.textView.textColor = UIColor(named: "Label")
                    ...
                    cell.textView.textColor = (cellStringInfo.cellTextColor != nil) ? cellStringInfo.cellTextColor : UIColor(named: "Label")
                    cell.backgroundColor = cellStringInfo.cellBackgroundColor ?? UIColor(named: "Tertiary_background")
                    cell.layer.borderColor = UIColor(named: "Border_color")?.cgColor
                ...
                case .Checkbox:
                    ...
                    cell.layer.borderColor = UIColor(named: "Border_color")?.cgColor
                    cell.backgroundColor = cellButtonInfo.cellBackgroundColor ?? UIColor(named: "Tertiary_background")
                    cell.layer.borderWidth = 0.5
                    return cell
                default:
